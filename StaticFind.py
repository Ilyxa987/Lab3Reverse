...
def GetStaticImportAddress(p: angr.Project):
    ...
    for func_node in cfg.kb.functions.values():
        for block in func_node.blocks:
            ...
            ins_addr = list(block._project.analyses.Disassembly(
                ranges=[(addr, addr + block.size)], thumb=block.thumb,
                block_bytes=block.bytes).raw_result_map["instructions"])
            if len(ins_addr) > 0:
                ins_addr = ins_addr[-1]
                a = str(list(block._project.analyses.Disassembly(
                    ranges=[(addr, addr + block.size)], thumb=block.thumb,
        ...
        for block in func_node.blocks:
            ...
            for i in range(len(ins_addr)):
                ...
                if 'push' in a:
                    argument = a
                if funcaddr in a:
                    print(hex(list(block._project.analyses.Disassembly(
                        ranges=[(addr, addr + block.size)],
                        thumb=block.thumb,
    ...
    for key, value in call_addresses.items():
        if value == hex(addr.rebased_addr):
            for func_node in cfg.functions.values():
                ...
                if (('print' in funcname or 'scanf' in funcname) and
                        func_node.addr < key < func_node.addr + func_node.size):
                    ...
                    #         if 'esi' in str(list(insns.values())[i].render())
                    #         and 'mov' in str(list(insns.values())[i].render()):
                    #             arguments.append(str(list(insns.values())[i].render()).split('[')[2].split(']')[0])
                else:
                    for block in func_node.blocks:
                        if block.addr <= key <= block.addr + block.size:
                            insns = block._project.analyses.Disassembly(
                                ranges=[(block.addr, block.addr + block.size)],
                                thumb=block.thumb,
                                block_bytes=block.bytes).raw_result_map["instructions"]
                            for i in range(len(insns.values()) - 1, -1, -1):
                                if ((funcname == 'puts' or funcname == 'gets_s') and
                                        'rcx' in str(list(insns.values())[i].render())
                                        or (funcname == 'WriteFile' or funcname == 'ReadFile')
                                        and 'rdx' in str(list(insns.values())[i].render())):
                                    arguments.append(str(list(insns.values())[i].render()).split('[')[2].split(']')[0])
    return arguments
