                ...
                for block in func_node.blocks:
                    if key >= block.addr and key <= block.addr + block.size:
                        ...
                        for i in range(len(insns.values())-1, -1, -1):
                            if 'rcx' in str(list(insns.values())[i].render()):
                                arguments.append(str(list(insns.values())[i].render()).split('[')[2].split(']')[0])
    return arguments

def Search(project: angr.Project):
    # Start in main()
    ...
    symb_vector = claripy.BVS('input', 14 * 8)
    initial_state.memory.store(0x7fffffffffeffb0, symb_vector)
    # Start simulation
    simulation = project.factory.simgr(initial_state)


    # Find the way yo reach the good address
    good_address = 0x14000108e
